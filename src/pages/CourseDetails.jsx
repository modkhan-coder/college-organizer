            ...
            <div className="card" style={{ minHeight: '400px' }}>
                ...
                {activeTab === 'study_guide' && (
                    <div style={{ position: 'relative', minHeight: '400px' }}>
                        {!isPremium && (
                            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backdropFilter: 'blur(5px)', background: 'rgba(255,255,255,0.8)', zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '12px' }}>
                                <Brain size={48} color="#f59e0b" style={{ marginBottom: '16px' }} />
                                <h2 style={{ marginBottom: '8px' }}>AI Study Guide</h2>
                                <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Unlock AI-powered summaries with Premium</p>
                                <button className="btn" style={{ background: '#f59e0b', color: 'white', border: 'none', padding: '10px 24px', borderRadius: '20px', fontWeight: 'bold' }} onClick={() => window.location.href = '/pricing'}>Get Premium</button>
                            </div>
                        )}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                                <select
                                    ...
                                    style={{ padding: '8px', fontSize: '0.9rem', width: '200px' }}
                                >
                                    <option value="all">Use All Materials</option>
                                    {files.map(f => <option key={f.id} value={f.name}>{f.name}</option>)}
                                </select>
                                <button className="btn btn-secondary" onClick={handleGenerateGuide} disabled={generating}>
                                    {generating ? <RefreshCw className="spin" size={18} /> : <Brain size={18} />}
                        ...
                        {studyGuide ? (
                            <div className="markdown-body" style={{
                                lineHeight: '1.8',
                                fontSize: '1.25rem', // Bolder font size for readability
                                color: 'var(--text-primary)'
                            }}>
                                <style>{`
                                    .katex { font-size: 1.15em !important; } 
                                    .katex-display { margin: 1.5em 0 !important; }
                            ...
                            <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-secondary)' }}>
                                ...
                                <p>Click generate to create a guide from your materials.</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                {activeTab === 'quiz' && (
                    <div style={{ position: 'relative', minHeight: '400px' }}>
                        {!isPremium && (
                            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backdropFilter: 'blur(5px)', background: 'rgba(255,255,255,0.8)', zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '12px' }}>
                                <HelpCircle size={48} color="#f59e0b" style={{ marginBottom: '16px' }} />
                                <h2 style={{ marginBottom: '8px' }}>AI Quiz Generator</h2>
                                <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Test your knowledge with AI quizzes (Premium)</p>
                                <button className="btn" style={{ background: '#f59e0b', color: 'white', border: 'none', padding: '10px 24px', borderRadius: '20px', fontWeight: 'bold' }} onClick={() => window.location.href = '/pricing'}>Get Premium</button>
                            </div>
                        )}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                        ...
                        {quiz ? (
                            <div style={{ fontSize: '1.15rem' }}>
                                ...
                                {quiz.questions?.map((q, idx) => (
                                    <div key={idx} style={{ marginBottom: '32px', padding: '24px', border: '1px solid var(--border)', borderRadius: '12px', background: 'var(--bg-app)' }}>
                                        <div style={{ fontWeight: 'bold', marginBottom: '16px', fontSize: '1.25rem', lineHeight: '1.5' }}>
                                        ...
                                        <details style={{ marginTop: '16px', fontSize: '1.05rem', color: 'var(--text-secondary)' }}>
                                            <summary style={{ cursor: 'pointer', fontWeight: '500' }}>Show Explanation</summary>
                            ...
                            <div style={{ textAlign: 'center', padding: '48px', color: 'var(--text-secondary)' }}>
                                <HelpCircle size={48} style={{ opacity: 0.5, marginBottom: '16px' }} />
                                <p>Generate a tailored quiz to test your knowledge.</p>
                            </div>
                        )}
                    </div>
                )}
                ...
                {activeTab === 'chat' && (
                    <div style={{ display: 'flex', flexDirection: 'column', height: '500px', position: 'relative' }}>
                        {!isPremium && (
                            <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', backdropFilter: 'blur(5px)', background: 'rgba(255,255,255,0.8)', zIndex: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '12px' }}>
                                <MessageSquare size={48} color="#f59e0b" style={{ marginBottom: '16px' }} />
                                <h2 style={{ marginBottom: '8px' }}>AI Tutor Chat</h2>
                                <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Chat with your course materials (Premium)</p>
                                <button className="btn" style={{ background: '#f59e0b', color: 'white', border: 'none', padding: '10px 24px', borderRadius: '20px', fontWeight: 'bold' }} onClick={() => window.location.href = '/pricing'}>Get Premium</button>
                            </div>
                        )}
                        <div style={{ flex: 1, overflowY: 'auto', marginBottom: '16px', padding: '16px', border: '1px solid var(--border)', borderRadius: '8px' }}>
                            {/* Chat Toolbar */}
                            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '12px' }}>
                                ...
                                >
                                    <option value="all">Source: All Materials</option>
                                    {files.map(f => <option key={f.id} value={f.name}>Source: {f.name}</option>)}
                                </select>
                            </div>

                            {chatHistory.length === 0 && <p style={{ color: 'var(--text-secondary)', textAlign: 'center' }}>Ask anything about your course documents.</p>}
                            {chatHistory.map((msg, idx) => (
                                <div key={idx} style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                        <div style={{ fontWeight: 'bold', fontSize: '1rem' }}>{msg.role === 'user' ? 'You' : 'AI'}:</div>
                                        {msg.role === 'assistant' && (
                                            <button
                                                ...
                                                title="Read Aloud"
                                            >
                                                {speakingIdx === idx ? <Square size={16} fill="currentColor" /> : <Volume2 size={16} />}
                                            </button>
                                        )}
                                    </div>
                            ...
                            {generating && <div style={{ color: 'var(--text-secondary)', fontStyle: 'italic' }}>AI is thinking...</div>}
                        </div>
                        <form onSubmit={handleChat} style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                            <div style={{ flex: 1, position: 'relative', display: 'flex', alignItems: 'center' }}>
                                <input
                                    className="input-field"
                                    value={chatInput}
                                    onChange={e => setChatInput(e.target.value)}
                                    placeholder={isListening ? "Listening..." : "Explain the concept of..."}
                                    disabled={generating}
                            ...
                            <button type="submit" className="btn btn-primary" disabled={generating || (!chatInput.trim() && !isListening)}>
                                Send
                            </button>
                        </form>
                    </div>
